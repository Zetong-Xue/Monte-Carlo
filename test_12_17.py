...
def MC(st, nmax, n, a_s):
	...
	def sobol_ran(col, row):
		list = sobol.sample(dimension=col, n_points=row)
		return(list)
	...
	def QMC_fail_rate(n, a_s, num_samples, r):
		...
		tot_1 = 0
		tot_2 = 0
		sobol_se = sobol_seq.i4_sobol_generate(r, num_samples)
		tot_p_list = []
		var_loop = []
		for t in range(num_samples):
			...
			for h in range(ftimes):
				vec = np.append(vec, get_rand_number(0,1))
	...
	f_max = list(range(1,nmax+1))

	for r in f_max:
		sample_tiems = st
		# print("this is loop r = ********************************", r)
		cost_ind = QMC_fail_rate(n, a_s, sample_tiems, r)[0]
		variance = QMC_fail_rate(n, a_s, sample_tiems, r)[1]
		...
		print("here is the total cost for 1 year", sum(cost_list), "with variance", sum(var_list)**0.5)

for e in range(1000, 4001, 1000):
	MC(e,3,1,5)
